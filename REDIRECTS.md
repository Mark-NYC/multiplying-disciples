...
publishing, not via redirect), and through Phase 4 Batch 1, which
migrated 7 tier-2 pages (see `PHASE_4_TIER_2_BATCH_PLAN.md`), again all
at their exact original real URLs with no slug changes, no merges, and
no collisions with existing hub or protected URLs. All URLs in
`URL_INVENTORY.md` are being preserved at their exact original path.
This file exists as the mechanism for the *future* case where a path
genuinely cannot be preserved (see rule 8 in the brief's SEO rules).
