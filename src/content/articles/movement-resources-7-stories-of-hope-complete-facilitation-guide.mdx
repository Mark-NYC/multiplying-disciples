...
notes: >
  ...
  now that the auto-generated "In This Article" TOC (14 real H2s in this
  article) supersedes it, and stripped all 17 in-body "---" horizontal
  rules, consistent with every other converted article — headings and
  card spacing already do the separating work.
  ...
  ArticleImage (Gen Z Bible study stock photo) in the opening section;
  ...
  only the mistake side); one PracticeCard for "Your Next Steps" (kept
  the real H2 heading above it, only the numbered list itself is boxed,
  same pattern as three-thirds' Week-1 PracticeCard); one PullQuote using
  the article's own closing line ("Hope isn't just available. Hope has a
  name—Jesus."), pulled up to the top of "The Bottom Line" section with
  the trailing sentence merged into the final paragraph. Deliberately
  left un-boxed: the "Person of Peace" definition (short, already a
  tight list), the "Three Equal Parts" 3/3rds framework definition
  (three-thirds.mdx leaves its own equivalent unboxed too, kept
  consistent across both), the "Critical Success Factors" and "What
  Happens After" sections (real H3 structure already, boxing would
  flatten it), the "Critical Rules" DO/DON'T list under Story 1 (the
  WarningCard budget was spent on the later, more general "What NOT to
  Do" list — this one stays as the existing bolded plain text, same
  restraint three-thirds applied to its ~8 non-promoted "Common mistake:"
  instances), and all 7 individual story write-ups (each already uses a
  repeating bold-label pattern — Who This Reaches / The Hope / the
  question — but the entries are full narrative paragraphs, not short
  reference facts, so RoleFact's compact 2-column grid was the wrong
  shape for them; left as plain H3 sections).
---
