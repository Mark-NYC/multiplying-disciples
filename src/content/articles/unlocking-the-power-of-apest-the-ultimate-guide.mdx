...
display_title: "Unlocking the Power of APEST"
...
notes: >
  ...
  2026-07-12 (spacing + highlighting pass): each role's deep-dive
  section was still a long, undifferentiated wall of text below the
  eyebrow pill. Two changes: (1) Apostle's "Overview of the Apostle"
  bullet list (Basic Definition/Effect/Characteristics/Shadow — the
  densest single reference block in the article) is now an InsightCard
  with accent="clay", the same treatment as the top role cards, so it
  reads as a highlighted reference block rather than a plain nested
  list. (2) The 4 recurring Q&A facts every role has ("Who is an NT
  example," "immature expressions," "if only this gift functioned,"
  "interdependence") are now wrapped in a new RoleFact component
  (src/components/article/RoleFact.astro) arranged in a 2x2
  `.fm-role-facts` grid, tinted to that role's accent — real headings
  (H4) are preserved inside each box for SEO, only the container is new
  styling. This replaces what was previously 4 back-to-back plain H4 +
  paragraph pairs with a highlighted, breathing-room-having grid.
  Normalized Teacher's 4th fact (the only one of the 5 roles' 4th
  facts that was a blockquote instead of a plain paragraph) to plain
  text so all 5 roles' grids look identical. Also converted 3 misused
  "pull quote as heading/blockquote" spots to real PullQuote: Apostle's
  closing H4 (word-for-word duplicate of a sentence in its own first
  paragraph), and the standalone blockquotes under Prophet and
  Evangelist.
---
...
<InsightCard label="Overview of the Apostle" accent="clay">
...
<PullQuote>The apostle is an entrepreneur of the gospel, willing to take risks and innovate new ways of reaching people with the message of salvation.</PullQuote>
...
<PullQuote>Prophets are not just talkers; they are doers. They live what they preach and model the values they espouse. They are not afraid to take a stand for what is right, even if it means going against the flow.</PullQuote>
...
<PullQuote>The evangelist is a "firestarter" who inspires and equips others to share their faith with those around them.</PullQuote>
