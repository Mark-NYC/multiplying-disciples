...
> **DO NOT LAUNCH without sign-off.** This checklist gates the
> DNS/hosting cutover decision. Status refresh (2026-07-12): the
> migration phases are complete — all tier-1/2/3 content is migrated,
> the unified article system is live in the repo, and the site builds
> and deploys on Vercel. The remaining gate is verifying the items
> below against the live WordPress site and getting explicit human
> approval for cutover. The Rollback section at the bottom of this
> file (formerly docs/archive/ROLLBACK_PLAN.md) is part of that gate.
...
- [ ] Complete protected URL inventory built from that export (extends
      `docs/archive/PROTECTED_URLS.md` / `URL_INVENTORY.md` beyond the current 45
      known URLs).
...
- [ ] All media URLs in `docs/archive/MEDIA_URLS_TO_PRESERVE.md` preserved (files
...
- [ ] 404 test: spot-check a sample of old URLs post-build; nothing
      ...
      `docs/archive/PHASE_8_FULL_SITE_AUDIT.md` §5.)
...
- [x] Internal link crawl (Phase 8, resolved Phase 9 — see
      `docs/archive/PHASE_9_LINK_TARGETS_AND_HUB_STATUS.md`): crawled all 98 pages.
...
- [ ] Lighthouse / basic speed check run on key templates (homepage,
      article, hub, tool).
- [ ] the Rollback section below reviewed and confirmed workable by whoever
...
## Archive URL checks (added Phase 6 — see `docs/archive/PHASE_6_ARCHIVE_URL_DECISION.md`)
...
remaining 68 blog posts — see `docs/archive/PHASE_10C_ARTICLE_FORMATTING_SYSTEM.md`
...
on the 8 named priority pages — see `docs/archive/PHASE_10B_VISUAL_POLISH.md` for
...
`docs/archive/PHASE_10_DESIGN_POLISH.md` for the full writeup. No new pages migrated,
...
(documented in `docs/archive/PHASE_10_DESIGN_POLISH.md`) — proceeded without them per
...
`docs/archive/PHASE_9_LINK_TARGETS_AND_HUB_STATUS.md`. Investigated all 6 real,
...
Rollback-section review, human go/no-go sign-off).
...
`docs/archive/PHASE_8_FULL_SITE_AUDIT.md` for the full 8-area breakdown. Result: 0
...
`docs/archive/PHASE_7B_TIER_3_BATCH_6.md`. `/contact-us/` received the special
...
`docs/archive/PHASE_7B_TIER_3_BATCH_5.md`. `/kingdom-ministry-training/` and
...
`docs/archive/PHASE_7B_TIER_3_BATCH_4.md`. `/beliefs-values/` and `/privacy-policy/`
...
`docs/archive/PHASE_7B_TIER_3_BATCH_3.md`. `/start-here/` and `/vision/` both
...
`docs/archive/PHASE_7B_TIER_3_BATCH_2.md`. `/starter-tools/` (dual-nav-linked from
...
`docs/archive/PHASE_7B_TIER_3_BATCH_1.md`. `/movement-resources/` migrated normally
...
`/blog/` is now built and live in the build — see `docs/archive/MIGRATION_PLAN.md`
...
per `docs/archive/PHASE_6_ARCHIVE_URL_DECISION.md`. 20 redirects proposed
...
- Protected URL inventory: built from real data, 122 published
  ...
  `docs/archive/PROTECTED_URLS.md`).
- Migrated: all 12 tier-1 URLs + all 22 tier-2 URLs (34 of 122, 100%
  ...
  can be checked.
- **Phase 5 protected URL audit (`docs/archive/PHASE_5_PROTECTED_URL_AUDIT.md`)
...
(Absorbed from docs/archive/ROLLBACK_PLAN.md, 2026-07-12. Current risk level:
no DNS cutover has happened; WordPress remains live and
authoritative, so there is nothing to roll back from yet.)

#### Principle

WordPress stays live and authoritative until the Astro site has been
verified against the full this checklist and a human has
...
3. **If something breaks post-cutover** (404s on real pages, ranking
   ...
   - WordPress itself is never deleted or modified during this
     ...
     the old site fully.
4. **After rollback**: diagnose the issue against this checklist
