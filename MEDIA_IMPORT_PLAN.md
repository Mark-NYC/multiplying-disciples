...
untracked size variants), `2025/02` (two uploads: first landed 132
files with only 1 of 9 tracked rows matched; a follow-up landed 3 more
of the 5 needed `apostles-meaning` files at full size — **2 files still
genuinely missing, only as `-100x100` thumbnails**: `5520c44066fbc694ac1c2e207151fe125162f59d.jpeg`
...
only ever received paths and metadata (via WordPress attachment
records and the GSC export) for the rest, never the actual image/PDF
bytes.
...
| `/wp-content/uploads/2025/02/Jesus-with-disciples-1024x513.jpg` | jpg | `/apostles-meaning-unlock-biblical-roles-greek-origins-and-modern-mission/` | Yes | 0 |
| `/wp-content/uploads/2025/02/Paul-in-Athens.jpg` | jpg | `/apostles-meaning-unlock-biblical-roles-greek-origins-and-modern-mission/` | Yes | 0 |
| `/wp-content/uploads/2025/02/jesus_chooses_twelve_apostles.webp` | webp | `/apostles-meaning-unlock-biblical-roles-greek-origins-and-modern-mission/` | Yes | 0 |
...
| 2026-07-06 | `2025/02` (follow-up) | 3 exact | **3 of the 5 needed `apostles-meaning` files now supplied at full size** — `Jesus-with-disciples-1024x513.jpg`, `Paul-in-Athens.jpg`, `jesus_chooses_twelve_apostles.webp` — flipped to "Yes" above. Confirmed via fresh `astro build` + `dist/` check that these 3 now resolve on the live page. **2 files remain genuinely missing**: `5520c44066fbc694ac1c2e207151fe125162f59d.jpeg` and `760px-Ghirlandaio_Domenico_-_Calling_of_the_Apostles_-_1481.jpg` — the follow-up upload still only contained `-100x100` thumbnail crops of these two exact filenames, not the full-size originals. **The `apostles-meaning` page now has 2 broken images (down from 5).** |
| 2026-07-06 | Decision | — | **Deferred, not blocking.** The user has decided the current state ("what I gave you works for now") is sufficient to move forward. The remaining 2 `apostles-meaning` broken images and the 3 `2025/02` `/stickers/` files (`2-1024x1024.jpg`, `3-Circles-Sticker-Multiplying-Disciples.webp`, `3.jpg`) will be examined and resolved later once the site is live, not chased further in this media-import phase. |
...
| 2026-07-06 | Phase 4 Tier 2 Batch 1 migration | n/a | No new uploads — this entry records that 7 tier-2 pages migrated in Phase 4 Batch 1 (see `PHASE_4_TIER_2_BATCH_PLAN.md`) now consume Priority C rows previously logged above as "not yet migrated": `/5-proven-strategies-unlock-the-secrets-of-effective-personal-evangelism/`, `/understanding-biblical-discipleship-a-complete-guide-to-following-jesus/`, `/four-fields-of-kingdom-growth-starting-and-releasing-healthy-churches-by-nathan-and-kari-shank/`, `/how-to-get-started-in-four-fields-training/`, `/disciple-making-movement-books-top-25-must-reads/`, `/prayer-is-essential-12-key-prayer-points-for-disciple-making-movements/`, `/seven-words-of-jesus-on-the-cross/`. All of these pages' tracked media rows were already marked "Yes" above and were reverified present via a fresh `astro build` + `dist/` check with zero broken images. |
| 2026-07-06 | Phase 4 Tier 2 Batch 2 migration | n/a | No new uploads — this entry records that 7 more tier-2 pages migrated in Phase 4 Batch 2 (see `PHASE_4_TIER_2_BATCH_PLAN.md`) now consume Priority C rows previously logged above as "not yet migrated": `/a-step-by-step-guide-to-prayer-walking-scriptures-and-changing-lives/`, `/what-is-a-strategy-coordinator-their-role-framework-and-impact-in-modern-missions/`, `/christian-prayer-wheel-a-structured-hour-of-prayer-that-actually-works/`, `/radical-discipleship-understanding-what-it-means-and-how-to-live-it/`, `/the-power-of-sharing-your-testimony-scripture-7-biblical-reasons-you-cant-stay-silent/`, `/the-high-price-of-discipleship-what-it-really-costs-to-follow-jesus/`, `/ridiculously-simple-3-ways-to-make-disciples/`. All 24 distinct tracked media rows across these 7 pages were already marked "Yes" above (the strategy-coordinator page alone accounts for 13 of them, its own dedicated `christian-prayer-wheel-sticker-1024x1024.webp` row stays tagged to `/stickers/`, not this page) and were reverified present via a fresh `astro build` + `dist/` check with zero broken images. This fully completes the Prayer hub (3/3 key articles), the Strategy Coordinator hub (1/1), and the Testimony hub (3/3). |
| 2026-07-06 | Phase 4 Tier 2 Batch 3 migration | n/a | No new uploads — this entry records that 7 more tier-2 pages migrated in Phase 4 Batch 3 (see `PHASE_4_TIER_2_BATCH_PLAN.md`): `/breakthrough-guide-for-a-modern-day-disciple/`, `/4-stages-of-movement-unlock-your-next-steps/`, `/the-power-of-multiplication-how-church-planting-movements-accomplish-the-great-commission/`, `/what-is-a-simple-church-meeting-christ-in-a-spiritual-family/`, `/love-and-obedience-exploring-the-biblical-relationship-between-the-two/`, `/disciple-making-movement-dmm-key-characteristics-and-definition/`, `/10-qualities-present-in-every-church-planting-movement-keys-to-sustainable-growth-and-multiplication/`. Of the 12 distinct tracked media rows across these 7 pages, 10 were already marked "Yes" above and reverified present via a fresh `astro build` + `dist/` check. The remaining row, `nonresident-JTW6AUbCLC4-unsplash-3-1.jpg`, stays "Won't fix" exactly as previously decided: `breakthrough-guide-for-a-modern-day-disciple` migrated with `og_image` omitted (its only media dependency, no inline images on the page), and `4-stages-of-movement-unlock-your-next-steps` needed it only as a decorative CTA background image that was removed as chrome during migration — neither page has a genuinely broken or missing visible image as a result. `love-and-obedience-exploring-the-biblical-relationship-between-the-two`'s tracked `75598-scaled.webp` row (already "Yes") was used directly as `og_image`, resolving the previously-flagged filename mismatch against WordPress's `75598.webp` thumbnail record — treated as the same source image at WordPress's auto-generated "-scaled" size, not a different photo. This fully completes the Church Planting Movements hub's tier-2 scope (5 of 8 listed slots; the remaining 3 are tier-3, out of this phase) and the Simple Church hub (3/3). |
...
- **Present so far: 165 of 171 tracked rows** (all 47 `2023/04` rows —
  ...
  `2025/01` rows — 2 A, 6 B, 15 C — plus 4 of 9 `2025/02` rows — 3 B,
  1 C — plus all 4 `2025/10` rows — 4 C — plus all 5 `2025/12` rows —
  5 C — plus all 4 `2025/03`/`2026/02`/`2020/12` rows — 3 B, 1 C. The
