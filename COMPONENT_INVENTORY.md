...
| `ArticleMediaBar.astro` | compact, dismissible sticky bar that smooth-scrolls to the article's own video/podcast/tool sections; third slot is contextual — Get the Guide crossfades once to Share in the final quarter of the article | media actions are in-page anchors (never link out); Share uses native `navigator.share` on mobile and copy-link with a "Copied" state on desktop; both swap halves share one grid cell so widths stay stable; buttons at a 44px tap-target floor; session-persistent dismissal; reduced-motion + focus + safe-area aware; `window.dataLayer` events (`_view/_watch/_listen/_guide/_share/_swap/_dismiss`). Cornerstone articles with 2+ media assets only; article-scoped MVP (mounted from the body, not the layout). See ARTICLE_SYSTEM.md "Supporting media" |
