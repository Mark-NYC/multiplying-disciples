...
| `ArticleMediaBar.astro` | compact, dismissible sticky bar (Watch / Listen / Get the Guide, plus an optional Share) that smooth-scrolls to the article's own video/podcast/tool sections | media actions are in-page anchors (never link out); optional Share uses native `navigator.share` on mobile (icon-only) and copy-link with a "Copied" state on desktop; session-persistent dismissal; reduced-motion + focus + safe-area aware; `window.dataLayer` events. Cornerstone articles with 2+ media assets only; article-scoped MVP (mounted from the body, not the layout). See ARTICLE_SYSTEM.md "Supporting media" |
